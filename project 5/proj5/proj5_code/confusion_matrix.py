...
def generate_confusion_data(
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        pd.extend(list(predicted_labels))
        tg.extend(list(target_data))

    targets = torch.Tensor(tg)
    preds = torch.Tensor(pd)

    class_labels = ["bedroom",
                    "coast",
                    "forest",
                    "highway",
                    "industrial",
...
def generate_confusion_matrix(
    ...
    if normalize:
        ...
        tg = list(targets)
        cnt = dict((x, tg.count(x)) for x in set(tg))
        # confusion_matrix = np.array(confusion_matrix)/np.array(list(cnt.values()))
        for key in cnt.keys(): 
            key = int(key)
            confusion_matrix[key] /= cnt[key]
