...
class PSPNet(nn.Module):
    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for layer in self.layer3: 
            layer.conv2 = nn.Conv2d(256, 256, kernel_size = (3, 3), stride = 1, 
                dilation = 2, padding = 2, bias = False)
            
            if layer.downsample != None: 
                layer.downsample = nn.Sequential(
                    nn.Conv2d(512, 256 * 4, kernel_size = (1,1), stride = 1, bias = False), 
                    nn.BatchNorm2d(256 *4)
                    )

        for layer in self.layer4: 
            layer.conv2 = nn.Conv2d(512, 512, kernel_size = (3, 3), stride = 1, 
                dilation = 4, padding = 4, bias = False)
            
            if layer.downsample != None: 
                layer.downsample = nn.Sequential(
                    nn.Conv2d(1024, 512 * 4, kernel_size = (1,1), stride = 1, bias = False), 
    ...
    def __create_classifier(self, in_feats: int, out_feats: int, num_classes: int) -> nn.Module:
        ...
        cls = nn.Sequential( 
            nn.Conv2d(in_feats, out_feats, kernel_size = (3,3), stride = 1, 
                padding =1, bias = False), 
            nn.BatchNorm2d(out_feats), 
            nn.ReLU(), 
            nn.Dropout(p = self.dropout), 
            nn.Conv2d(out_feats, num_classes, kernel_size = (1,1), stride = 1, 
                bias = False)
            )
    ...
    def forward(
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        aux_output = x # F.interpolate(x, size=(x_size[2], x_size[3]), mode = 'bilinear')
        main_output = self.layer4(x)
        # print("aux: ", aux_output.shape)
        # print("main: ", main_output.shape)
        
        if self.use_ppm:
            main_output = self.ppm.forward(main_output)

        aux_output = self.aux(aux_output)
        main_output = self.cls(main_output)
        # print("aux: ", aux_output.shape)
        # print("main: ", main_output.shape)

        aux_output = F.interpolate(aux_output, size=(x_size[2], x_size[3]), 
            mode = 'bilinear', align_corners=True)
        logits = F.interpolate(main_output, size=(x_size[2], x_size[3]), 
            mode = 'bilinear', align_corners=True)
