...
def get_magnitudes_and_orientations(
    ...
    for i in range(m):
        for j in range(n):
            orientations[i,j] = np.arctan2(Iy[i,j], Ix[i,j])
...
def get_gradient_histogram_vec_from_patch(
    ...
    for i in range(4): 
        for j in range(4):
            mag_cell = window_magnitudes[(4*i):(4*(i+1)),(4*j):(4*(j+1))]
            ori_cell = window_orientations[(4*i):(4*(i+1)), (4*j):(4*(j+1))]
            hist = np.histogram(ori_cell, bins = 8, range = (-np.pi, np.pi), weights = mag_cell)
            wgh = np.append(wgh, hist[0]) #np.append(wgh, [hist[0]], 1)
...
def get_feat_vec(
    ...
    # print(feature_width, (int)(feature_width/2))
    # print(x, y)
    mag_grid = magnitudes[y-(feature_width-1)//2:y+feature_width//2+1, 
        x-(feature_width-1)//2:x+feature_width//2+1]
    ori_grid = orientations[y-(feature_width-1)//2:y+feature_width//2+1, 
        x-(feature_width-1)//2:x+feature_width//2+1]
    # print(mag_grid.shape)
    # print(ori_grid.shape)
    
    fv = get_gradient_histogram_vec_from_patch(mag_grid, ori_grid)
    norm = np.linalg.norm(fv.reshape(1, (int)(feature_width/4)**2*8))
    if (norm != 0):
