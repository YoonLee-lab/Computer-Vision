...
def calculate_mccnn_cost_volume(
    ...
    small = block_size // 2
    cost_volume = torch.zeros(H - 2 * small, W - 2 * small, max_search_bound)

    for y in range(small, H - small):
          for x in range(small, W - small):
              D = torch.ones(max_search_bound,) * 255

              offset = min(x - small, max_search_bound - 1) 
              # TODO once per pixel not disparity
              for disp in range(0, offset + 1): 
                patch = left_img[y - small : y+ small + 1, x - small : x + small + 1].reshape(1, C, H, W)
                window = right_img[y - small : y+ small + 1, x - small - disp : x + small - disp + 1].reshape(k, C, H, W)
                err = sim_measure_function(MCNET(ws=ws).classifier, patch, window)
                D[disp] = err
...
def mc_cnn_similarity(
    ...
    batch = torch.zeros(k, C * H * W * 2)
    for i in range(k):
        sample = cnn_search_window[i]
        cnn_patch = cnn_patch.unsqueeze(0).unsqueeze(0).reshape(1,1,ws,ws)
        sample = sample.unsqueeze(0).reshape(1,1,ws,ws)
        x = torch.cat((cnn_patch,sample),0).view(-1)
        batch[i] = x
    
    cnn_similarity = fc_layers(batch.to(device))    
