...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    return np.reshape(np.array(kernel),(ksize, 1))
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel =  np.outer(create_Gaussian_kernel_1D(k, cutoff_frequency), create_Gaussian_kernel_1D(k, cutoff_frequency))
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    padded = np.pad(image, ((height,),(width,),(0,)), 'constant', constant_values = 0)
    ...
    for channel in range(image.shape[2]):
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                ...
                filtered_image[i, j, channel] = np.sum((np.multiply(kernel, filter)), axis = (0,1))
